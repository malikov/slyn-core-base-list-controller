...
var Controller = function(backboneView, model, options) {
    ...
    this.loadCached = (options && options.cached)? options.loadCached : false;
    this.cachedData = (options && options.cachedData)? options.cachedData : false;
    ...
    this.clickItem = function(elt){
        this.cachedData = {
			url: window.location.pathname,
			items: this.view.renderedItems,
			collection: this.view.collection,
			verticalScrollPosition: $(window).scrollTop(),
			expiration: 20000
		};
		
		Store.set(this.cachedData.url, this.cachedData);
    };
};
        ...
        .then(function(models) {
            this.view = PageManager.instantiateView({
                ...
                params: {
                    ...
                    data: models,
                    params: $.extend(params, {infinite: this.infinite, cached: this.cached, cachedData: this.cachedData})
                }
            });
